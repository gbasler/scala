...
trait MatchAnalysis extends MatchApproximation {
  ...
  // debug output:
        ...
        try {
          ...
          println(counterExamples.mkString(", "))
          val relevant = counterExamples.filterNot(_ == NoExampleHidden)
//          val pruned = counterExamples.map(_.toString).distinct.sorted
          val pruned = CounterExample.prune(relevant.sortBy(_.toString)).map(_.toString).sorted
            ...
            val res = prunedEqualTo match {
              ...
              case _ if cls != NoSymbol && !isPrimitiveValueClass(cls) &&
                ...
                def args(brevity: Boolean = beBrief) = {
                  ...
                  examples match {
                    case Seq(NoExample, l: ListExample) =>
