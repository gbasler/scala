            ...
            val res = prunedEqualTo match {
              ...
              case _ if cls != NoSymbol && !isPrimitiveValueClass(cls) &&
                ...
                def args(brevity: Boolean = beBrief) = {
                  ...
                  examples match {
                    case List(Some(NoExample), Some(l: ListExample)) => Some(List(WildcardExample, l))
                    case ex                                          => sequence(ex)
                  }
                }

                cls match {
                  case ConsClass                                => args().map(ListExample)
                  case _ if isTupleSymbol(cls)                  => args(brevity = true).map(TupleExample)
                  case _ if cls.isSealed && cls.isAbstractClass => None
