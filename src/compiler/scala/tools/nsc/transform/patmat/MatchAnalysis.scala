...
trait MatchAnalysis extends MatchApproximation {
  ...
  // debug output:
    ...
    def expandModel(solution: Solution): List[Map[Var, (Seq[Const], Seq[Const])]] = {
      ...
      val grouped: Map[Var, List[Sym]] = solution.unassigned.groupBy(_.variable)

      // expand each group
      val expanded = for {
        (group, syms) <- grouped.toList
      } yield {

        val (equal, notEqual) = varAssignment.getOrElse(group, Nil -> Nil)

        def addVarAssignment(equalTo: List[Const], notEqualTo: List[Const]) = {
          Map(group ->(equal ++ equalTo, notEqual ++ notEqualTo))
        }
      ...
      if (expanded.isEmpty) {
        ...
        // TODO: do we need some limits here?
